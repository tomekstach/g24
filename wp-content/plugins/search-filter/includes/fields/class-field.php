...
class Field extends Record_Base
{
     ...
     * @var array
     */
    private $render_escape_callbacks = [];
    ...
    {
        $parsed_setting_support = [];

        foreach (static::$setting_support as $setting_name => $support) {
            $parsed_setting_support[$setting_name] = [];
            ...
            if (is_bool($support)) {
                ...
                // should be met.
                $parsed_setting_support[$setting_name]['conditions'] = [
                    'relation' => 'OR',
                    'rules'    => [
                        [
                            'relation' => 'AND',
    ...
    public static function add_setting_support_condition($setting_support, $setting_name, $new_conditions, $is_required = true)
    {
        ...
        $conditions = [
            ...
            'rules'    => [
                [
                    'relation' => 'AND',
                    'rules'    => [], // Push here to add required conditions
                ],
    ...
    {
        return apply_filters('search-filter/field/url_template', [], $this);
    }
     ...
     */
    public function apply_wp_query_args($query_args = [])
    {
    ...
    public function get_style_attribute($name)
    {
        if (isset($this->attributes[$name])) {
            ...
            $style = Style::find(['id' => $this->get_calc_styles_id()]);
     ...
     */
    public static function create($attributes = [], $context = [])
    {
    ...
    public static function find($conditions, $return_type = 'object')
    {
        ...
        if (empty($query->items)) {
            return new \WP_Error('search_filter_field_not_found', __('Field not found.', 'search-filter'), ['status' => 404]);
        }
        ...
        if ($return_type === 'object') {
            ...
            foreach ($query->items as $item) {
                $fields[] = Field_Factory::create_from_record($item);
            }
            return $fields;
        }
        // AstoSoft - end
