...
class Fields
{
     ...
     * @var array
     */
    private static $active_field_configs = [];
    ...
    public static function init()
    {
        ...
        add_action('search-filter/record/save', [__CLASS__, 'save_css'], 10, 2);

        // Register settings.
        add_action('init', [__CLASS__, 'register_settings'], 2);
    }
    ...
    public static function shortcode($attributes)
    {
        ...
        if (is_numeric($attributes['field'])) {
            ...
            // AstoSoft - start
            // If the name is empty, we can't do anything.
            if ('' === $conditions['name']) {
                unset($conditions['name']);
                if (isset($attributes['query']) && '' !== $attributes['query']) {
                    $conditions['query_id'] = absint($attributes['query']);
                }
                $fields = Field::find($conditions, 'array');
                if (is_wp_error($fields)) {
                    $output = $fields->get_error_message();
                } else {
                    // If we have multiple fields, we need to render them all.
                    $output = '';
            ...
            if ($return_as === 'objects') {
                foreach ($query->items as $record) {
                    try {
                        ...
                        $fields[] = new \WP_Error('invalid_field', $e->getMessage(), ['status' => 400]);
                    }
                }
